...
import 'package:nunta_app/Provider_Screens/provider_comment_to_manage.dart';
...
class EvaluationManage extends StatefulWidget {
  final id;
  const EvaluationManage({Key key, @required this.id}) : super(key: key);

  @override
  _EvaluationManageState createState() => _EvaluationManageState(id);
}

class _EvaluationManageState extends State<EvaluationManage> {
  String id;
  _EvaluationManageState(this.id);
  ...
  Future<dynamic> fetchEvaluation() async {
    data = [];
    var response =
        await APIProvider().get(url: "provider/items/evaluate", params: {
      "api_token": userToken,
      "id": id,
    });
    if (response != null && response['status'] == "success") {
      ...
      for (var i = 0; i < response['result'].length; i++) {
        ...
        var oneThing = ProCommentToManage(
  ...
  Future<dynamic> deleteEvaluate(String id) async {
    var response =
        await APIProvider().get(url: "provider/items/evaluate/remove", params: {
      "api_token": userToken,
      "id": id,
    });
    if (response != null && response['status'] == "success") {
