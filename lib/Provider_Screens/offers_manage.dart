...
class OffersManage extends StatefulWidget {
  const OffersManage({Key key}) : super(key: key);

  @override
  _OffersManageState createState() => _OffersManageState();
}

class _OffersManageState extends State<OffersManage> {
  List data = [];
  final userToken = getIt<UserAccountModel>().userToken;
  Future<dynamic> fetchItems() async {
    data = [];
    var response =
        await APIProvider().get(url: "provider/items/show_offers", params: {
      "api_token": userToken,
    });
  ...
  Future<dynamic> deleteProviderItem(String id) async {
    ...
    if (response != null && response['status'] == "success") {
      ...
      fetchItems();
      return ScaffoldMessenger.of(context).showSnackBar(SnackBar(backgroundColor: Colors.red[900],
          content: SnackBarText("offer has been deleted Successfully")));
    } else {
      print(response);
      String errMessage =
      ...
      bottomNavigationBar: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            ProviderNavBar(selectedMenu: MenuState.providerOffers),
          ],
        ),
      ),
      appBar: buildAppBar(),
